...
{
    public partial class FrmMainCaoSu : Form
    {
        ...
        private bool _isUISyncing = false;  
        private readonly object _uiSyncLock = new object();  
        ...
        public bool IsFullHD
        {
            ...
            set
            {
                ...
                else //Show control for full Resolution screen
                {
                    ...
                    // Thiết lập cơ bản cho TableLayoutPanel1
                    tableLayoutPanel1.RowCount = 2;
                    tableLayoutPanel1.ColumnCount = 1;
                    tableLayoutPanel1.SuspendLayout(); // Tối ưu hiệu năng khi thay đổi giao diện

                    // Xác định điều kiện hiển thị
                    bool isDatabaseMode = _SelectedJob.CompareType == CompareType.Database;
                    bool isAppearResultTable = ProjectLabel.IsNutrifood && !Shared.Settings.IsManufacturingMode;

                    // Xóa sạch Style cũ để thiết lập lại từ đầu
                    tableLayoutPanel1.RowStyles.Clear();

                    if (isAppearResultTable)
                    {
                        // TRƯỜNG HỢP NUTRIFOOD: Hàng 0 chiếm 100%, Hàng 1 biến mất
                        tableLayoutPanel1.RowStyles.Add(new RowStyle(SizeType.Percent, 100F));
                        tableLayoutPanel1.RowStyles.Add(new RowStyle(SizeType.Percent, 0F));

                        if (!tableLayoutPanel1.Controls.Contains(pnlDatabase))
                            tableLayoutPanel1.Controls.Add(pnlDatabase, 0, 0);

                        // tableLayoutPanel1.Controls.Remove(pnlCheckedResult); // Nếu có pnlCheckedResult
                    }
                    else if (isDatabaseMode)
                    {
                        // TRƯỜNG HỢP DATABASE: Chia đôi hoặc tùy chỉnh (Ví dụ 50-50)
                        // Nếu bạn muốn Database hiện FULL khi ở mode này, hãy đổi 50F thành 100F
                        tableLayoutPanel1.RowStyles.Add(new RowStyle(SizeType.Percent, 100F));
                        tableLayoutPanel1.RowStyles.Add(new RowStyle(SizeType.Percent, 0F));
                    ...
                    {
                        // TRƯỜNG HỢP CÒN LẠI: Ẩn Database, hiện các thành phần khác ở hàng 1
                        tableLayoutPanel1.RowStyles.Add(new RowStyle(SizeType.Percent, 0F));
                        tableLayoutPanel1.RowStyles.Add(new RowStyle(SizeType.Percent, 100F));
                    ...
                    // Đảm bảo pnlDatabase luôn lấp đầy ô được giao
                    if (pnlDatabase != null) pnlDatabase.Dock = DockStyle.Fill;
        ...
        public FrmMainCaoSu(frmJobCaoSu parentForm)
        {
            InitializeComponent();
        ...
        // ── Ngăn nút X tắt app, thay bằng quay về frmJobCaoSu ───────────────
        private bool _isForceClose = false;

        protected override void OnFormClosing(FormClosingEventArgs e)
        {
            if (!_isForceClose && e.CloseReason == CloseReason.UserClosing)
            {
                // Kiểm tra nếu đang sync UI
                bool isSyncing;
                lock (_uiSyncLock)
                {
                    isSyncing = _isUISyncing;
                }
                    ...
                    Task.Run(() =>
                    {
                        this.Invoke(new Action(() =>
                        {
                            var result = CustomMessageBox.Show(
                                "Hệ thống đang đồng bộ dữ liệu. Vui lòng đợi hoàn tất trước khi đóng!",
                                "Thông báo",
                ...
                if (isRunning || Shared.OperStatus == OperationStatus.Running || Shared.OperStatus == OperationStatus.Processing)
                {
                    e.Cancel = true;
                    ...
                    Task.Run(() =>
                    {
                        ...
                        {
                            var result = CustomMessageBox.Show(
                                "Vui lòng dừng chương trình trước khi đóng!",
                                "Thông báo",
                                MessageBoxButtons.OK,
                ...
                // Nếu không có vấn đề gì, cho phép đóng
                e.Cancel = true;
                _isForceClose = true;
        ...
        private void FrmMain_FormClosed(object sender, FormClosedEventArgs e)
        {
            ...
            {
                // Luôn hiện lại frmJobCaoSu dù ReleaseResource() có lỗi hay không
        ...
        private void InitControls()
        {
            ...
            HideControls(btnHistory); // lblStatusCamera01 // lblSensorControllerStatus
            ...
            //LOTNumberTextBox.Text = _SelectedJob.LOTNumber ?? string.Empty;
            //MaterialName.Text = _SelectedJob.CaoSuProduct.product_name?? string.Empty;
            numberOfCode.Text = _SelectedJob.NumberTotalsCode.ToString() ?? "";
            PalletNumberCombo.Items.Clear();
            PalletNumberCombo.Items.AddRange(_SelectedJob.PalletList.Select(p => p.QrCode).ToArray());
            PalletNumberCombo.SelectedIndex = _SelectedJob.PalletList.Count != 0 ? 0 : -1;
        ...
        private void InitEvents()
        {
            ...
            // FIX: Đăng ký đúng event từ CaoSuVerificationSender
            // CaoSuVerificationSender raise OnSyncCheckDataParameterChange (không phải OnSyncDataParameterChange)
            ...
            Resize += (obj, e) =>
            {
                ...
                {
                    if (IsFullHD) IsFullHD = false;
                }
                else if (Size.Width >= 850 || Size.Height >= 850)
                {
                    if (!IsFullHD) IsFullHD = true;
                }
        ...
        // FIX: Handler đúng cho Check SaaS event (từ CaoSuVerificationSender)
        // Đọc từ _SentCheckedCodeObtainFromFile (file check), KHÔNG phải file print
        private void Shared_OnSyncCheckDataParameterChange(object sender, EventArgs e)
        {
                ...
                if (sender is SyncDataParams ParamsName)
                {
                    switch (ParamsName.DataType)
                    {
                        case SyncDataType.SaaSSuccess:
                            // Lazy-load file check nếu chưa có
                            ...
                            // Cập nhật trạng thái dòng tương ứng trong file check
                            ...
                            // Đếm lại từ file check để hiển thị số đồng bộ thành công
                            ...
                            Shared.NumberOfCheckSentSaaS = successCount;

                            // Cập nhật UI label đồng bộ check
                            if (SentSaaS.InvokeRequired)
                                SentSaaS.Invoke(new Action(() => SentSaaS.Text = $"Mã check đồng bộ SaaS: {successCount}"));
                            else
                                SentSaaS.Text = $"Mã check đồng bộ SaaS: {successCount}";
                            break;
            ...
            {
                ProjectLogger.WriteError("Shared_OnSyncCheckDataParameterChange error: " + ex.Message);
            }
        }
        private void FrmMainCaoSu_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (isRunning)
            {
                ...
                e.Cancel = true;
                _isForceClose = false; // Reset để lần bấm X tiếp theo vẫn đi qua đúng flow
                return;
            }
        }
        private void AddBarcodes(object sender, EventArgs e)
        {
            try
            {
                ...
                if (_SelectedJob.PalletList.Count >= 4)
                {
                    //CustomMessageBox.Show("Đã đủ số lượng mã Pallet!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
                bool exists = _SelectedJob.PalletList.Any(p => p.QrCode == qrCode);
                if (exists)
                {
                    CustomMessageBox.Show("Mã pallet đã tồn tại!", "Bị trùng lặp", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }

                PalletNumberCombo.UI(() =>
                {
                    PalletNumberCombo.Items.Insert(0, qrCode);
                    PalletNumberCombo.SelectedIndex = 0;
                });

                _SelectedJob.PalletList.Insert(0,
                    new Model.CaoSuDongNai.PalletModel
                    {
                        QrCode = qrCode
                    }
                );
            ...
            {
                CustomMessageBox.Show("Không thể thêm Palelt!", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void UpdatedCodeWithPalletNumber()
        {
            ...
            var codes = SyncDataList.ReadPrintedCodeData(CommVariables.PathSentDataPallet + _SelectedJob.CheckedResultPath);
            int successCount = codes.Count(c => c.Length > 4 && string.Equals(c[4], "success", StringComparison.OrdinalIgnoreCase));
            int total = _SelectedJob.productWeight > 30 ? 144 : 240;

            CodeWithPallet.Text = $"{successCount} / {total}";
        }
        ...
        public void InitDataGridView(DataGridView dgv, string[] columns, int imgIndex = -1, bool isPOD = false)
        {
            ...
            // Đặt chế độ tự động co giãn cột để lấp đầy chiều rộng DataGridView
            ...
            for (int index = 0; index < columns.Length; index++)
            {
                if (index == imgIndex && imgIndex != -1)
                {
                    var col = new DataGridViewImageColumn
                    {
                        ...
                        Width = 60 // hoặc giá trị phù hợp cho cột hình ảnh
                    };
                    col.DefaultCellStyle.NullValue = null;
                    col.SortMode = DataGridViewColumnSortMode.NotSortable;
                    col.HeaderCell.Style.Alignment = DataGridViewContentAlignment.MiddleCenter;
                ...
                if (interactOnUI)
                {
                    switch (checkPrinterSettings)
                    {
                        case CheckPrinterSettings.NotRawData:
                            CustomMessageBox.Show(Lang.DataTypeMustBeRAWData, Lang.Info, MessageBoxButtons.OK, MessageBoxIcon.Information); break;
                        case CheckPrinterSettings.PODNotEnabled:
                            CustomMessageBox.Show(Lang.PODFeatureIsNotEnabled, Lang.Info, MessageBoxButtons.OK, MessageBoxIcon.Information); break;
                        case CheckPrinterSettings.ResponsePODDataNotEnable:
                            CustomMessageBox.Show(Lang.ResponsePODDataFeatureIsNotEnable, Lang.Info, MessageBoxButtons.OK, MessageBoxIcon.Information); break;
                        case CheckPrinterSettings.ResponsePODCommandNotEnable:
                            CustomMessageBox.Show(Lang.ResponsePODCommandFeatureIsNotEnable, Lang.Info, MessageBoxButtons.OK, MessageBoxIcon.Information); break;
                        case CheckPrinterSettings.MonitorNotEnable:
                            CustomMessageBox.Show(Lang.MonitorFeatureIsNotEnabled, Lang.Info, MessageBoxButtons.OK, MessageBoxIcon.Information); break;
                        case CheckPrinterSettings.PODMode:
                            ...
                            CustomMessageBox.Show(mes, Lang.Info, MessageBoxButtons.OK, MessageBoxIcon.Information); break;
                    }
                }
                isRunning = false;
                return;
        ...
        private void Compare(CancellationToken token)
        {
            ...
            catch (OperationCanceledException)
            {
                ...
                {
                    // Already cancelled and disposed by ReleaseResource() — safe to ignore
                }
        ...
        private void UpdateUIPrintedResponse(CancellationToken token)
        {
            ...
            catch (System.OperationCanceledException)
            {
                ...
                try { _BackupResponseCancelTokenSource?.Cancel(); } catch (ObjectDisposedException) { }
        ...
        private void UpdateUICheckedResult(CancellationToken token)
        {
            ...
            catch (OperationCanceledException)
            {
                ...
                Thread.Sleep(20);
                try { _BackupImageCancelTokenSource?.Cancel(); } catch (ObjectDisposedException) { }
                try { _BackupResultCancelTokenSource?.Cancel(); } catch (ObjectDisposedException) { }
        ...
        private void StartPrintedDataProcess(PrintingQueueProcessor PrintedProcess)
        {
            try
            {
                ...
                if (Shared.UserPermission.isOnline)
                {
                    lock (_uiSyncLock)
                    {
                        _isUISyncing = true;  // Bắt đầu sync
                    }
            ...
            catch (Exception)
            {
                lock (_uiSyncLock)
                {
                    _isUISyncing = false;  // Reset nếu có lỗi
                }
            }
        }
        ...
        private void StopPrintedDataProcess(PrintingQueueProcessor PrintedProcess)
        {
            try
            {
                ...
                if (Shared.UserPermission.isOnline)
                {
                    ...
                    // Delay một chút để UI cập nhật hoàn toàn
                    Task.Delay(500).ContinueWith(_ =>
                    {
                        lock (_uiSyncLock)
                        {
                            _isUISyncing = false;  // Kết thúc sync
                        }
                    });
                }
            }
        ...
        private async void NewExportCheckedResultToFile(CancellationToken token)
        {
            ...
            try
            {
                ...
                string pathPallet = CommVariables.PathSentDataPallet + _SelectedJob.CheckedResultPath;
                ...
                try
                {
                    ...
                    //_palletDataProcess = ReliableProcessorFactory.CreateCaoSuPalletProcessor(sentPalletDataPath, url, pathPallet);
                ...
                while (true)
                {
                    ...
                    if (valueArr.Count() > 0)
                    {
                        ...
                        try
                        {
                            if (clone[0][2] == "Valid")
                            {
                                // Index, Qrcdoe, Status, Processing Time, Datetime
                                _checkedDataProcess.Enqueue(int.Parse(clone[0][0]), clone[0]);
                                //   _palletDataProcess.Enqueue(int.Parse(clone[0][0]), clone[0]);
        ...
        private async void StopProcess(bool interactOnUI = true, string messages = "", bool isClosed = false, bool isManualClose = false)
        {
            ...
            if (interactOnUI)
            {
                ...
                int NumberOfPalletSent = _SelectedJob.PalletList.Count(x => x.IsSent);
            ...
            try { _UIPrintedResponseCancelTokenSource?.Cancel(); } catch (ObjectDisposedException) { }
            try { _OperationCancelTokenSource?.Cancel(); } catch (ObjectDisposedException) { }
            while (_QueueBufferDataObtained.TryDequeue(out _)) { }
            while (_QueuePositionDataObtained.TryDequeue(out _)) { }
            _QueueBufferUpdateUIPrinter.Enqueue(null);
        ...
        {
            try { _VirtualCTS?.Cancel(); } catch (ObjectDisposedException) { }
            try { _UICheckedResultCancelTokenSource?.Cancel(); } catch (ObjectDisposedException) { }
            try { _UIPrintedResponseCancelTokenSource?.Cancel(); } catch (ObjectDisposedException) { }
            try { _BackupImageCancelTokenSource?.Cancel(); } catch (ObjectDisposedException) { }
            try { _BackupResultCancelTokenSource?.Cancel(); } catch (ObjectDisposedException) { }
            try { _BackupResponseCancelTokenSource?.Cancel(); } catch (ObjectDisposedException) { }
            try { _BackupSendLogCancelTokenSource?.Cancel(); } catch (ObjectDisposedException) { }
            try { _BackupRSFPLogCancelTokenSource?.Cancel(); } catch (ObjectDisposedException) { }
        ...
        private void ReleaseResource()
        {
            ...
            {
                try { _VirtualCTS?.Cancel(); } catch (ObjectDisposedException) { }
                try { _SendDataToPrinterTokenCTS?.Cancel(); } catch (ObjectDisposedException) { }
                try { _PrinterRespontCST?.Cancel(); } catch (ObjectDisposedException) { }
                ...
                _PrinterRespontCST = null;

                // Các CTS "operation group" bạn đang comment Cancel/Dispose là OK (để tránh race) — giữ nguyên.
        ...
        private void UpdateJobInfo(JobModel jobModel)
        {
            ...
            numberOfCode.Text = jobModel.NumberTotalsCode.ToString();

            // ── Danh sách pallet ──────────────────────────────────────────────────
            PalletNumberCombo.Items.Clear();
            if (jobModel.PalletList != null && jobModel.PalletList.Count > 0)
            {
                PalletNumberCombo.Items.AddRange(jobModel.PalletList.Select(p => p.QrCode).ToArray());
                PalletNumberCombo.SelectedIndex = 0;
            }
            else
            {
                PalletNumberCombo.SelectedIndex = -1;
            }

            // ── Mã đã đồng bộ với pallet ─────────────────────────────────────────
            UpdatedCodeWithPalletNumber();
        }
        ...
        private void lblCheckedResult_Click(object sender, EventArgs e)
        {
        ...
        private void EnableUIComponent(OperationStatus operationStatus, bool isNonStart = false)
        {
            ...
            btnAddBarcode.Enabled = isEnable;
            // END menu script
            if (isEnable)
            {
                ProcessUserAccess();
