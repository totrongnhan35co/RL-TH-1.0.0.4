...
{
    public partial class FrmMainCaoSuCheck : Form
    {
        #region VARIABLES DEFINITION
        private readonly frmJobCaoSu _ParentForm = null;
        ...
        private string _JobCheckFilePath;
        private string _JobPrintFilePath;
            ...
            set
            {
                ...
                else //Show control for full Resolution screen
                {
                    ...
                    if (this.tableLayoutPanelCheckedResult.RowCount <= 1)
                    {
                        ...
                        btnHistory.Visible = btnAccount.Visible = true;
        ...
        public FrmMainCaoSuCheck(
       frmJobCaoSu parentForm,
       JobModel selectedJob,
       string lotNumber,
       string productCode,
       string jobCheckFilePath)
        {
            InitializeComponent();
            ...
            _JobCheckFilePath = jobCheckFilePath;

            try { LOTNumberTextBox.Text = _SelectedJob?.LOTNumber ?? ""; }
            catch { LOTNumberTextBox.Text = ""; }

            try
            {
                MaterialName.Text = !string.IsNullOrWhiteSpace(_SelectedJob?.LastCheckedProductName)
                    ? _SelectedJob.LastCheckedProductName
                    : (!string.IsNullOrWhiteSpace(_SelectedJob?.LastCheckedProductCode)
                        ? _SelectedJob.LastCheckedProductCode
                        : "");
            }
            catch { MaterialName.Text = ""; }

            try { numberOfCode.Text = _SelectedJob?.NumberTotalsCode.ToString() ?? "0"; }
            catch { numberOfCode.Text = "0"; }

            try { txtJobName.Text = _SelectedJob?.FileName ?? ""; }
            catch { txtJobName.Text = ""; }

            try { lblJobName.Text = _SelectedJob?.FileName ?? ""; }
            catch { lblJobName.Text = ""; }

            // Hiển thị trọng lượng đã chọn từ dgvItems
            try { txtWeightCheck.Text = _SelectedJob?.productWeight > 0 ? _SelectedJob.productWeight.ToString() : ""; }
            catch { txtWeightCheck.Text = ""; }

            try { UpdateJobInfo(_SelectedJob); }
            catch { }
        }
        ...
        private void FrmMainCaoSu_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (isStarted)
            {
                CustomMessageBox.Show(
                    "Vui lòng dừng chương trình trước khi đóng cửa sổ!",
                    "Thông báo",
                    MessageBoxButtons.OK,
                    MessageBoxIcon.Warning
                );
                e.Cancel = true; // Chặn đóng form
            }
            // Không gọi Application.Exit() hoặc Close() ở đây!
        }
        ...
        private void InitControls()
        {
            ...
            //// ===== ẨN TÍNH NĂNG IN CHO JOB CHECK =====
            //// Ẩn panel trạng thái in (Received, Printed, Sent)
            //tableLayoutPanelPrintedState.Visible = false;

            //// Ẩn nút Trigger (chỉ dùng camera, không cần trigger thủ công khi in)
            ////btnTrigger.Visible = false;

            //// Ẩn panel thông tin mã đang in
            //pnlPrintedCode.Visible = false;

            //// Ẩn các thông tin đồng bộ in (SaaS, Sent data)
            pnlSentData.Visible = false;
            pnlReveied.Visible = false;
            //// Ẩn nút thêm mã (chỉ dùng khi in)
            btnAddBarcode.Visible = false;
            numPrinterPort.Visible = false;
            label1.Visible = false;
            //// Ẩn nút database (chỉ xem database in)
            //btnDatabase.Visible = false;

            //// Ẩn nút export data (export dữ liệu in)
            //btnExportData.Visible = false;
            //btnExportAll.Visible = false;

            //// Ẩn nhãn printer status
            //lblStatusPrinter01.Visible = false;

            // Ẩn label đồng bộ SaaS in
            //SentSaaS.Visible = true;
            //sentSaaSSuccess.Visible = true;
            //SyncDataLabel.Visible = true;
            //syncDataBtn.Visible = true;
            //SyncLoading.Visible = true;
            SyncDataText.Visible = true;
            // ==========================================
            ...
            // Luôn ẩn lại sau khi ProcessUserAccess gán
            btnExportData.Visible = false;
            btnExportAll.Visible = false;

            btnAccount.Enabled = Shared.UserPermission.Accounts;
            pnlControllButton.Enabled = Shared.UserPermission.Controls;
            txtWeightCheck.Text = _SelectedJob.productWeight > 0
               ? _SelectedJob.productWeight.ToString()
               : "";
            HideControls(btnHistory);
            LineName.Text = "Tên Line: " + Shared.Settings.RLinkName ?? "";
            pnlCurrentCheck.Text = "Thông tin mã kiểm tra";
            pnlVerificationProcess.Text = "Vận hành kiểm tra";
            UserNameDisplay.Text = "Trạng thái: " + (Shared.UserPermission.isOnline ? "Online" : "Offline");

            SyncDataLabel.Text = Lang.SyncData;
            syncDataBtn.Text = Lang.Sync;
            LOTNumberTextBox.Text = _SelectedJob.LOTNumber;
            numberOfCode.Text = _SelectedJob.NumberTotalsCode.ToString();
            PalletNumberCombo.Items.Clear();

            // Load lại danh sách pallet đã quét từ job
            if (_SelectedJob.PalletList != null && _SelectedJob.PalletList.Count > 0)
            {
                foreach (var pallet in _SelectedJob.PalletList)
                {
                    if (!string.IsNullOrWhiteSpace(pallet.QrCode))
                        PalletNumberCombo.Items.Add(pallet.QrCode);
                }
                PalletNumberCombo.SelectedIndex = 0;
            }

            StopPrintedDataProcess(_printedDataProcess);
            if (_checkedDataProcess != null) _checkedDataProcess.Stop();
            if (_palletDataProcess != null) _palletDataProcess.Stop();
        ...
        private void InitEvents()
        {
            ...
            this.FormClosing += FrmMainCaoSu_FormClosing;
        ...
        private void AddBarcodes(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (_SelectedJob.PalletList == null)
                    _SelectedJob.PalletList = new List<Model.CaoSuDongNai.PalletModel>();

                bool exists = _SelectedJob.PalletList.Any(p => p.QrCode == qrCode);
                if (exists)
                {
                    CustomMessageBox.Show("Mã pallet đã tồn tại!", "Bị trùng lặp",
                        MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }

                _SelectedJob.PalletList.Add(
                    new Model.CaoSuDongNai.PalletModel { QrCode = qrCode });

                // Cập nhật Shared.CurrentJob TRƯỚC — loop CaoSuPalletSender
                // đọc Shared.CurrentJob.PalletList để check IsSent == false
                Shared.CurrentJob = _SelectedJob;
                _SelectedJob.SaveAsCheckedFile();

                PalletNumberCombo.UI(() =>
                {
                    PalletNumberCombo.Items.Add(qrCode);
                    PalletNumberCombo.SelectedIndex = PalletNumberCombo.Items.Count - 1;
                });

                // Nếu _palletDataProcess chưa chạy (chưa có mã trước) → khởi động
                // Nếu đã chạy → loop tự detect PalletList thay đổi và gửi API
                if (_palletDataProcess == null)
                    CreatePalletProcesssor(_SelectedJob);
            }
            catch (Exception ex)
            {
                ProjectLogger.WriteError("AddBarcodes (pallet) error: " + ex.Message);
            }
        }
        ...
        //    try
        //    {
        //        if (!(sender is SyncDataParams ParamsName)) return;

        //        switch (ParamsName.DataType)
        //        {
        //            case SyncDataType.SaaSSuccess:
        //                // Lazy-load nếu chưa có dữ liệu
        //                if (_SentCheckedCodeObtainFromFile == null || _SentCheckedCodeObtainFromFile.Count == 0)
        //                {
        //                    if (!string.IsNullOrWhiteSpace(_SelectedJob.CheckedResultPath))
        //                    {
        //                        string sentCheckPath = CommVariables.PathSentDataChecked + _SelectedJob.CheckedResultPath;
        //                        _SentCheckedCodeObtainFromFile = FileFuncs.ReadCodeData(sentCheckPath);
        //                    }
        //                    else
        //                    {
        //                        _SentCheckedCodeObtainFromFile = new List<string[]>();
        //                    }
        //                }

        //                // Cập nhật in-memory dòng tương ứng (CodeIndex = Id = index 1-based trong file)
        //                int idx = ParamsName.CodeIndex - 1;
        //                if (idx >= 0 && idx < _SentCheckedCodeObtainFromFile.Count
        //                    && _SentCheckedCodeObtainFromFile[idx].Length > PrintingValues.SaaSStatus)
        //                {
        //                    _SentCheckedCodeObtainFromFile[idx][PrintingValues.SaaSStatus] = "success";
        //                }

        //                // Đếm lại từ in-memory để hiển thị chính xác
        //                int successCount = _SentCheckedCodeObtainFromFile.Count(
        //                    item => item.Length > PrintingValues.SaaSStatus &&
        //                            item[PrintingValues.SaaSStatus].Equals("success", StringComparison.OrdinalIgnoreCase));

        //                CheckSaaSSuccess = Shared.NumberOfCheckSentSaaS = successCount;
        //                break;
        ...
        private void Shared_OnSyncCheckDataParameterChange(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    case SyncDataType.SaaSSuccess:
                        if (string.IsNullOrWhiteSpace(_SelectedJob.CheckedResultPath)) break;

                        string sentCheckPath = CommVariables.PathSentDataChecked + _SelectedJob.CheckedResultPath;

                        // Đọc lại file mỗi lần để đảm bảo chính xác
                        // (file vừa được MarkAsSent bởi CaoSuVerificationSender)
                        var lines = FileFuncs.ReadCodeData(sentCheckPath);
                        _SentCheckedCodeObtainFromFile = lines;

                        int successCount = lines.Count(
                            item => item.Length > VerifyingValues.SaaSStatus &&
                ...
                if (sender is SyncDataParams ParamsName)
                {
                    switch (ParamsName.DataType)
                    {
                        case SyncDataType.SaaSSuccess:
                            ...
                            break;
                        case SyncDataType.PalletSuccess:
                            UpdatedCodeWithPalletNumber();
                            break;
        ...
        private void UpdatedCodeWithPalletNumber()
        {
            if (InvokeRequired)
            {
                Invoke(new Action(UpdatedCodeWithPalletNumber));
                return;
            }

            try
            {
                // Tính từ PalletList: số pallet đã IsSent=true × số mã mỗi pallet
                // Đáng tin cậy hơn đọc file pallet (file có thể sai do GetPalletSize bug)
                double weight = _SelectedJob.productWeight;
                int perPalletSize = weight > 30 ? 36 : (weight > 0 ? 60 : 36);

                int sentPalletCount = _SelectedJob.PalletList != null
                    ? _SelectedJob.PalletList.Count(p => p.IsSent)
                    : 0;

                int successCount = sentPalletCount * perPalletSize;

                CodeWithPallet.Text = $"{successCount} / {GetPalletTotal()}";
            }
            catch (Exception)
            {
            }
        ...
        /// <summary>
        /// Tổng số mã toàn bộ job (mẫu số của CodeWithPallet).
        /// Ví dụ: 4 pallet × 36 = 144 → trả về 144.
        /// </summary>
        private int GetPalletTotal()
        {
            double weight = _SelectedJob.productWeight;
            if (weight > 30) return 144;  // 4 pallet × 36 mã
            if (weight > 0) return 240;   // 4 pallet × 60 mã

            // weight = 0: dùng NumberTotalsCode = tổng toàn job (144)
            double numberTotals = _SelectedJob.NumberTotalsCode;
            if (numberTotals > 0)
                return (int)numberTotals;
        ...
        //private void UpdatedCodeWithPalletNumber()
        //{
        //    if (CodeWithPallet.InvokeRequired)
        //    {
        //        CodeWithPallet.Invoke(new Action(UpdatedCodeWithPalletNumber));
        //        return;
        //    }
        //    string sentPalletDataPath = CommVariables.PathSentDataPallet + _SelectedJob.CheckedResultPath;
        //    var _CheckedPrintedCodeObtainFromFile = SyncDataList.ReadPrintedCodeData(sentPalletDataPath);
        //    int NumberOfSentCodeWithPallet = _CheckedPrintedCodeObtainFromFile.Count(item => item.Length > 4 &&
        //                                          item[4].Equals("success", StringComparison.OrdinalIgnoreCase));
        //    int quantity = _SelectedJob.productWeight > 30 ? 144 : 240;
        //    CodeWithPallet.Text = $"{NumberOfSentCodeWithPallet} / {quantity}";
        //}
        ...
        private void DebugVirtual()
        {
            ...
            btnVirtualStart.Visible = false;
            btnVirtualStop.Visible = false;
        ...
        private void StartProcess(bool interactOnUI = true)
        {
            ...
            {
                CustomMessageBox.Show(checkInitDataMessage, Lang.Error, MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            ...
            if (checkCondition != CheckCondition.Success)
            {
                if (interactOnUI)
                {
                    if (checkCondition == CheckCondition.NotConnectCamera)
            ...
            // ← QUAN TRỌNG: gán Shared.CurrentJob để CaoSuPalletSender dùng đúng job
            Shared.CurrentJob = _SelectedJob;

            if (Shared.CaoSuAllValueProcess == null)
                Shared.CaoSuAllValueProcess = new CaoSuAllValueProcess(_SelectedJob);

            // Kiểm tra nếu có mã + pallet chưa gửi từ session trước
            CheckAndAssignUnassignedCodesToPallet();
        ...
        /// Khi Start job: kiểm tra file pallet, nếu có mã "NotSent" đủ palletSize
        /// và chưa có pallet chưa gửi → tạo pallet mới tự động và gửi API
        /// </summary>
        private void CheckAndAssignUnassignedCodesToPallet()
        {
            try
            {
                if (string.IsNullOrWhiteSpace(_SelectedJob.CheckedResultPath)) return;
                TryTriggerPalletSync();
                double weight = _SelectedJob.productWeight;
                int palletSize = weight > 30 ? 36 : 60;

                string sentPalletDataPath = CommVariables.PathSentDataPallet + _SelectedJob.CheckedResultPath;
                if (!File.Exists(sentPalletDataPath)) return;

                // Đọc mã chưa được xử lý pallet (NotSent)
                var unsentLines = FileFuncs.ReadCodeData(sentPalletDataPath)
                    .Where(item => item.Length > VerifyingValues.SentStatus &&
                                   item[VerifyingValues.SentStatus]
                                       .Equals("NotSent", StringComparison.OrdinalIgnoreCase))
                    .ToList();

                if (unsentLines.Count < palletSize) return;

                // Kiểm tra đã có pallet chưa gửi chưa
                bool hasUnsentPallet = _SelectedJob.PalletList != null &&
                                       _SelectedJob.PalletList.Any(p => !p.IsSent);
                if (hasUnsentPallet) return;

                // Chưa có pallet chưa gửi nhưng đủ mã → cần tạo pallet mới
                // Thông báo cho user biết
                ProjectLogger.WriteInfo($"CheckAndAssignUnassignedCodesToPallet: {unsentLines.Count} mã chưa có pallet, cần quét thêm pallet mới.");
            }
            catch (Exception ex)
            {
                ProjectLogger.WriteError("CheckAndAssignUnassignedCodesToPallet error: " + ex.Message);
            }
        }
        //private void StartProcess(bool interactOnUI = true)
        //{
        ...
        //        return;
            ...
            //    // JobCheck: chỉ camera check — không gửi lệnh đến printer
            //    _SelectedJob.JobStatus = JobStatus.Unfinished;
            //    _SelectedJob.SaveAsCheckedFile();
            ...
            //    // Chỉ chạy camera compare — KHÔNG chạy ExportPrintedResponse / UpdateUIPrintedResponse
            //    Shared.OperStatus = OperationStatus.Running;
            //    CompareAsync(operationToken);
            ...
            //    // KHÔNG gọi:
            //    // ExportPrintedResponseToFileAsync  — dành cho in
            //    // UpdateUIPrintedResponseAsync       — dành cho in
            //    // SendDataToPrinterAsync             — dành cho in
            //    // BackupResultFinishPrintCommandAsync — dành cho in
        ...
        private void NewExportPrintedResponseToFile(CancellationToken token)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    StopPrintedDataProcess(_printedDataProcess);
                    _printedDataProcess = ReliableProcessorFactory.CreateCaoSuPrintingProcessor(sentDataPath, "", dataPath);
        ...
        private VerificationQueueProcessor _palletDataProcess;
        ...
        private async void NewExportCheckedResultToFile(CancellationToken token)
        {
            if (_SelectedJob.CheckedResultPath == "")
            {
                ...
                if (!File.Exists(path))
                {
                    using (StreamWriter sw = new StreamWriter(path, true, new UTF8Encoding(true)))
                        sw.WriteLine(String.Join(",", _ColumnNames));
                }
            ...
            try
            {
                ...
                string sentPalletPath = CommVariables.PathSentDataPallet + _SelectedJob.CheckedResultPath;
                ...
                // Khởi tạo processor — chỉ 1 lần khi Start, không restart trong loop
                if (_checkedDataProcess != null) _checkedDataProcess.Stop();
                _checkedDataProcess = ReliableProcessorFactory.CreateCaoSuVerificationProcessor(sentDataPath, url, path);
                _checkedDataProcess.Start();

                // Pallet processor chạy liên tục, tự kiểm tra queue đủ palletSize thì gửi
                CreatePalletProcesssor(_SelectedJob);
                ...
                while (true)
                {
                    ...
                    if (valueArr.Count > 0)
                    {
                        ...
                        if (clone[0][2] == "Valid")
                        {
                            ...
                            // Ghi mã vào file pallet (NotSent) để pallet processor xử lý
                            _palletDataProcess?.EnqueuePallet(int.Parse(clone[0][0]), clone[0]);

                            try
                            {
                                DateTime dt = DateTime.Parse(clone[0][4]);
                                Shared.CaoSuAllValueProcess?.UpdateCheck(clone[0][1], dt.ToString("yyyy-MM-dd HH:mm:ss"));
                            }
                            catch { }
                        }
                    }
        ...
        /// <summary>
        /// Kiểm tra: nếu có pallet chưa gửi (IsSent=false) VÀ đủ mã NotSent
        /// → restart pallet processor để gửi API ngay.
        /// Gọi sau mỗi lần: quét pallet mới hoặc check được mã Valid.
        /// </summary>
        // ─────────────────────────────────────────────────────────────
        // Hàm trung tâm: kiểm tra điều kiện và gán mã vào pallet + gửi API
        // Gọi sau: Start job, quét pallet mới, camera check mã Valid
        // ─────────────────────────────────────────────────────────────
        private void TryTriggerPalletSync()
        {
            try
            {
                if (string.IsNullOrWhiteSpace(_SelectedJob.CheckedResultPath)) return;
                if (_SelectedJob.PalletList == null || !_SelectedJob.PalletList.Any(p => !p.IsSent)) return;
                ...
                int unsentCount = FileFuncs.ReadCodeData(sentPalletDataPath)
                    .Count(item => item.Length > VerifyingValues.SentStatus &&
                                   item[VerifyingValues.SentStatus]
                                       .Equals("NotSent", StringComparison.OrdinalIgnoreCase));

                if (unsentCount < palletSize) return;

                // Đủ điều kiện: processor đã chạy sẽ tự xử lý khi queue đủ
                // Nếu chưa chạy (mở lại job) thì khởi động
                if (_palletDataProcess == null)
                    CreatePalletProcesssor(_SelectedJob);
            }
            catch (Exception ex)
            {
                ProjectLogger.WriteError("TryTriggerPalletSync error: " + ex.Message);
            }
        }
        //private void CreatePalletProcesssor(JobModel jobModel)
        //{
        //    try
        //    {
        ...
        //        if (_palletDataProcess != null) _palletDataProcess.Stop();
        //        _palletDataProcess = ReliableProcessorFactory.CreateCaoSuPalletProcessor(sentPalletDataPath, "", pathPallet);
        //        _palletDataProcess.Start();
        //    }
        //    catch (Exception)
        //    {
        //    }
        ...
        private void CreatePalletProcesssor(JobModel jobModel)
        {
            try
            {
                if (string.IsNullOrWhiteSpace(jobModel.CheckedResultPath)) return;

                string sentPalletDataPath = CommVariables.PathSentDataPallet + jobModel.CheckedResultPath;

                if (_palletDataProcess != null) _palletDataProcess.Stop();
                _palletDataProcess = ReliableProcessorFactory.CreateCaoSuPalletProcessor(
                    sentPalletDataPath, "", sentPalletDataPath);
                _palletDataProcess.Start();

                // ── Load lại mã chưa được gán pallet khi mở lại job ──────────
                // Chỉ enqueue mã từ file CHECK có SentStatus = "NotSent"
                // VÀ chưa tồn tại trong file PALLET (tránh append duplicate)
                string sentCheckDataPath = CommVariables.PathSentDataChecked + jobModel.CheckedResultPath;
                if (!File.Exists(sentCheckDataPath)) return;

                // Lấy tập hợp Id đã có trong file pallet
                var existingPalletIds = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
                if (File.Exists(sentPalletDataPath))
                {
                    foreach (var palletLine in FileFuncs.ReadCodeData(sentPalletDataPath))
                    {
                        if (palletLine.Length > VerifyingValues.Index &&
                            !string.IsNullOrWhiteSpace(palletLine[VerifyingValues.Index]))
                            existingPalletIds.Add(palletLine[VerifyingValues.Index]);
                    }
                }

                // Chỉ enqueue mã chưa có trong file pallet
                var unsentChecked = FileFuncs.ReadCodeData(sentCheckDataPath)
                    .Where(item =>
                        item.Length > VerifyingValues.SentStatus &&
                        item[VerifyingValues.SentStatus].Equals("NotSent", StringComparison.OrdinalIgnoreCase) &&
                        !existingPalletIds.Contains(item[VerifyingValues.Index]))
                    .ToList();

                foreach (var line in unsentChecked)
                {
                    if (!int.TryParse(line[VerifyingValues.Index], out int id)) continue;
                    _palletDataProcess.Enqueue(id, line);
                }
            }
            catch (Exception ex)
            {
                ProjectLogger.WriteError("CreatePalletProcesssor error: " + ex.Message);
            }
        }
        ...
        private async void StopProcess(bool interactOnUI = true, string messages = "", bool isClosed = false, bool isManualClose = false)
        {
            if (interactOnUI)
            {
                ...
                int NumberOfSentCodeWithPallet = _CheckedPrintedCodeObtainFromFile.Count(item => item.Length > 4 &&
                                                      item[4].Equals("success", StringComparison.OrdinalIgnoreCase));
                int test = _CheckedPrintedCodeObtainFromFile.Count - NumberOfSentCodeWithPallet;


              
                int NumberOfPalletSent = (_SelectedJob.PalletList != null)
                        ? _SelectedJob.PalletList.Count(x => x.IsSent)
                        : 0;
                if (NumberOfPalletSent < 4)
                {
                    this.Invoke((MethodInvoker)delegate
                    {
                        dialogResult = CustomMessageBox.Show(
                            $"Chỉ có {NumberOfPalletSent}/4 pallet được đồng bộ " +
                            $"\nCó {test} mã chưa được đồng bộ" +
                            $"\nBạn có muốn dừng chương trình không ?", Lang.Confirm, MessageBoxButtons.YesNo, MessageBoxIcon.Question);
        ...
        private async void InitDataAsync(JobModel jobModel)
        {
            ...
            {
                #region Sent Printed Data — guard khi path rỗng hoặc file chưa tồn tại
                ...
                {
                    _SentPrintedCodeObtainFromFile = new List<string[]>();
                }

                _SelectedJob.NumberOfSaaSSentCodes = SaaSSuccess = _SentPrintedCodeObtainFromFile
                    .Count(item => item.Length > 4
                        && item[4].Equals("success", StringComparison.OrdinalIgnoreCase));
                ...
                _SelectedJob.NumberOfCheckSaaSSentCodes = CheckSaaSSuccess = _SentCheckedCodeObtainFromFile
                    .Count(item => item.Length > 4
                        && item[4].Equals("success", StringComparison.OrdinalIgnoreCase));

                try { UpdatedCodeWithPalletNumber(); }
                catch { }
                if (!string.IsNullOrWhiteSpace(_SelectedJob.CheckedResultPath))
                {
                    CreatePalletProcesssor(_SelectedJob);
                ...
                // Validate database trước khi load
                if (string.IsNullOrWhiteSpace(jobModel.DirectoryDatabase)
                    || !File.Exists(jobModel.DirectoryDatabase))
                {
                    Invoke(new Action(() =>
                    {
                        MessageBox.Show(
                            "Không tìm thấy file database của JobCheck!\nPath: "
                            + jobModel.DirectoryDatabase,
                ...
                Task<List<string[]>> databaseTsk = InitDatabaseAndPrintedStatusAsync(jobModel);
                ...
                if (checkInitDataMessage != "")
                {
                    foreach (string value in checkInitDataMessage.Split('\n'))
                    {
                        if (value != "")
                        {
                            CuzAlert.Show(value, Alert.enmType.Error,
                                new Size(500, 120), new Point(Location.X, Location.Y), this.Size, true);
                        }
                    }
                }
                else
                {
                    ...
                    if (_PrintedCodeObtainFromFile.Count() > 1)
                    {
                        ...
                        _SelectedJob.NumberOfPrintedCodes = NumberPrinted =
                            _PrintedCodeObtainFromFile.Where(x => x[1] == "Printed").Count();

                        int firstWaiting = _PrintedCodeObtainFromFile
                            .IndexOf(_PrintedCodeObtainFromFile.Find(x => x[1] == "Waiting"));
                        ...
                        {
                            CuzAlert.Show(
                                Lang.DuplicateDataMessage.Replace("NN", "" + _NumberOfDuplicate) + Lang.PODFormat,
                                Alert.enmType.Warning,
                                new Size(500, 120), new Point(Location.X, Location.Y), Size, false);
                        }
                    }
                }
            }
        ...
        private void InitCompareData(List<string[]> datas, List<string[]> result)
        {
            ...
            try
            {
                // Build tập hợp các mã đã được check Valid
                ...
                if (datas.Count > 0)
                {
                    ...
                    {
                        // ── CHỈ cho phép check mã đã in (Printed / Reprint) ──────────
                        // Mã "Waiting" = chưa in → KHÔNG được phép check Valid
                        // Áp dụng cho CẢ JobPrint lẫn JobCheck (StandAlone)
                        string printStatus = datas[index][1];
                        if (printStatus != "Printed" && printStatus != "Reprint")
                            continue;
                        ...
                        if (_CheckedResultCodeSet.Contains(data))
                        {
                            // Đã check Valid trước đó → Status = true (đã sử dụng)
                        ...
                        {
                            // Đã in nhưng chưa check → Status = false (chờ camera check)
        ...
        private async void ActionChanged(object sender, EventArgs e)
        {
            ...
            else if(sender == btnAddBarcode)
            {
                try
                {
                    ...
                    var result = CustomMessageBox.Show(
                        $"Bạn có chắc chắn muốn tạo thêm {NumberOfNewCode} mã mới không ?",
                        Lang.Confirm,
                        MessageBoxButtons.YesNo,
                        MessageBoxIcon.Question);
                    if (result != DialogResult.Yes) return;

                    List<string> list = Base30AutoCodeGenerator.GenerateCodesForCaoSuDongNai(quantity: NumberOfNewCode);
                    ...
                    if (Shared.UserPermission.isOnline)
                    {
                        ...
                        bool isSent = await _ParentForm.SendGeneratedCodes(firstColumnList, _SelectedJob);
                        if (!isSent)
                        {
            ...
            else if (sender == btnDatabase || sender == pnlPrintedCode)
            {
                ...
                if (_FormPreviewDatabase == null || _FormPreviewDatabase.IsDisposed)
                {
                    ...
                    string sentDataPath = CommVariables.PathSentDataPrinted + _SelectedJob.PrintedResponePath;
                    var SentPrintedCodeObtainFromFile = FileFuncs.ReadCodeData(sentDataPath);
            ...
            else if(sender == syncDataBtn)
            {
                ...
                //    StopPrintedDataProcess(_printedDataProcess);
                //    _printedDataProcess = ReliableProcessorFactory.CreateCaoSuPrintingProcessor(sentDataPath, url, dataPath);
                //    StartPrintedDataProcess(_printedDataProcess);

                //    string urlChecked = ManufacturingApis.postCheckedDataUrl();
                //    string sentCheckedDataPath = CommVariables.PathSentDataChecked + _SelectedJob.CheckedResultPath;
                //    if (_checkedDataProcess != null) _checkedDataProcess.Stop();
                //    _checkedDataProcess = ReliableProcessorFactory.CreateCaoSuVerificationProcessor(sentCheckedDataPath, urlChecked, dataPath);
                //    _checkedDataProcess.Start();
                //}
                //else
                //{
        ...
        private void UpdateJobInfo(JobModel jobModel)
        {
            ...
            // Hiển thị trọng lượng từ jobModel (đã được lưu từ ComboBox cell listweight)
            txtWeightCheck.Text = jobModel.productWeight > 0
                ? jobModel.productWeight.ToString()
                : "";
        }
        ...
        private void btnJob_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                // Đóng tất cả FrmMainCaoSuCheck đang mở
                var openChecks = Application.OpenForms.OfType<FrmMainCaoSuCheck>().ToList();
                foreach (var f in openChecks)
                {
                    try { f.Close(); } catch { /* optional logging */ }
                }

                // Show/hide form job chính nếu cần
                this.Show(); // nếu đây là form job chính
            }
            catch (Exception ex)
            {
                ProjectLogger.WriteError("Error in BtnJob_Click: " + ex.Message);
            }
        ...
        private void EnableUIComponent(OperationStatus operationStatus, bool isNonStart = false)
        {
            ...
            // Controls in — luôn ẩn/disable trong JobCheck
            ...
            {
                ProcessUserAccess();
                // Đảm bảo các controls in vẫn ẩn sau ProcessUserAccess
                btnExportData.Visible = false;
                btnExportAll.Visible = false;
                btnAddBarcode.Visible = false;
                tableLayoutPanelPrintedState.Visible = false;
        ...
        void EnableUIComponentWhenLoadData(bool isEnable)
        {
            ...
            btnHistory.Enabled = isEnable;
            btnSettings.Enabled = Shared.UserPermission.Settings; //  isEnable && 
