...
{
    public partial class FrmJobTH : Form
    {
        ...
        private FrmMainTH _FormMainPC = null;
            ...
            else if (sender == btnNext)
            {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        {
                            _FormMainPC = new FrmMainTH(this);
                            _FormMainPC.Show();
                        }
                        else
                        {
        ...
        //    // Load LineName from settings
        //    LoadLineName();

        //    // Subscribe to QR code received event from server
        //    SubscribeToQRCodeEvents();
        //}
        private void FrmJob_Load(object sender, EventArgs e)
        {
            LoadJobNameList();
            ...
            // Khởi tạo ComboBox chọn cột barcode
            InitColumnNameControl();

            // Nếu DB đang connected và đã có table được chọn → load cột ngay
            if (_isDatabaseConnected && cmbTableName.SelectedItem != null)
                PopulateColumnNames(cmbTableName.SelectedItem.ToString());
        }
        ...
        private void InitEvents()
        {
            ...
            // LineName textbox event
            LineName.TextChanged += LineName_TextChanged;
        }
        ...
        private void SaveJob()
        {
            ...
            {
                // === Validation ===
                if (cmbTableName.SelectedItem == null || string.IsNullOrEmpty(cmbTableName.Text))
                {
                    CuzMessageBox.Show("Vui lòng chọn bảng dữ liệu!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return;
                }

                if (cmbColumnName == null || cmbColumnName.SelectedItem == null)
                {
                    CuzMessageBox.Show("Vui lòng chọn trường barcode cần in!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return;
                }

                string tableName = cmbTableName.SelectedItem.ToString();
                string columnName = cmbColumnName.SelectedItem.ToString();
                ...
                // === Export dữ liệu từ DB ra file CSV ===
                string fileName = $"{tableName}_{columnName}_{DateTime.Now:yyyyMMdd_HHmmss}.csv";
                ...
                // Show loading
                UIControlsFuncs.UI(imageLoading, () => { imageLoading.Visible = true; imageLoading.BringToFront(); imageLoading.Refresh(); });
                ...
                using (var writer = new StreamWriter(filePath, false, Encoding.UTF8))
                {
                    ...
                    using (IDbConnection connection = GetDatabaseConnection(connectionString))
                    {
                        ...
                        {
                            command.CommandText = databaseType.ToLower() == "sql"
                                ? $"SELECT [{columnName}] FROM [{tableName}] WHERE [{columnName}] IS NOT NULL AND LTRIM(RTRIM(CONVERT(NVARCHAR, [{columnName}]))) != ''"
                                : $"SELECT `{columnName}` FROM `{tableName}` WHERE `{columnName}` IS NOT NULL AND TRIM(`{columnName}`) != ''";
                ...
                Console.WriteLine($"[SaveJob] Exported {rowCount} rows from {tableName}.{columnName} → {filePath}");

                // Cập nhật UI
                txtDirectoryDatabse.Text = _JobModel.DirectoryDatabase = Shared.databasePath = filePath;
                _NumberTotalsCode = rowCount;
                numberOfCodes.Text = $"{rowCount} mã";

                // === Tạo JobModel ===
                _JobModel = InitJobModel();
                if (_JobModel != null)
                {
                    _JobModel.NumberTotalsCode = rowCount;
                    _JobModel.TableName = tableName;
                    _JobModel.BarcodeColumnName = columnName;
                    _JobModel.IsFirstRowHeader = true;
                    ...
                    if (_JobModel.PrinterSeries && _JobModel.CompareType == CompareType.Database)
                    {
                    ...
                    if (Shared.CheckJobHasExist(JobName))
                    {
                        ...
                        if (tmpJob != null)
                        {
                            if (tmpJob.JobStatus == JobStatus.Deleted)
                            {
                                ...
                                try { File.Move(oldJobPath, newJobPath); }
                                catch { }
                            }
                            else
                            {
                ...
                DialogResult dialogResult = CuzMessageBox.Show(
                    $"Đã xuất {rowCount} mã từ [{tableName}].[{columnName}].\n" + Lang.SuccessfulNewJobCreationStartTheProcess,
        ...
        //        string fileName = $"Test_{DateTime.Now:yyyyMMdd_HHmmss}.csv";
        ...
        //            _JobModel.BarcodeColumnName = cmbColumnName?.SelectedItem?.ToString() ?? "";
        //            _JobModel.FileName = DateTime.Now.ToString("yyMMdd_HHmmss") + "_LINE" + "_" + RandomCharHelper.GetThreeRandomCharacters();
        ...
        private void cmbTableName_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                string tableName = cmbTableName.SelectedItem.ToString();
                Shared.Settings.CenterIndiaModel.TableName = tableName;
                Shared.SaveSettings();
                PopulateColumnNames(tableName);
            }
        }

        #endregion Monitor_Database

        #region LineName Management

        /// <summary>
        /// Load LineName from SettingModel and display in textbox
        /// </summary>
        private void LoadLineName()
        {
            if (InvokeRequired)
            {
                Invoke(new Action(() => LoadLineName()));
                return;
            }
            ...
            {
                Console.WriteLine($"[frmJobTH] Error loading LineName: {ex.Message}");
            }
        }

        /// <summary>
        /// Save LineName to SettingModel when textbox changes
        /// </summary>
        private void LineName_TextChanged(object sender, EventArgs e)
        {
            try
            {
                Shared.Settings.LineName = LineName.Text;
                Shared.SaveSettings();
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[frmJobTH] Error saving LineName: {ex.Message}");
            }
        }

        #endregion LineName Management

        #region QR Code Data Display
        
        /// <summary>
        /// Subscribe to QR Code received events from the API server
        /// </summary>
        private void SubscribeToQRCodeEvents()
        {
            try
            {
                if (Shared.ServerService != null && Shared.ServerService != null)
                {   
                    Shared.ServerService.QRCodeReceived -= OnQRCodeReceived;
                    Shared.ServerService.QRCodeReceived += OnQRCodeReceived;
                    Console.WriteLine("[frmJobTH] Subscribed to QR Code events");
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[frmJobTH] Error subscribing to QR Code events: {ex.Message}");
            }
        }

        /// <summary>
        /// Handle QR Code received event from API
        /// </summary>
        private void OnQRCodeReceived(object sender, Services.THMilk.Controller.QRCodeReceivedEventArgs e)
        {
            if (InvokeRequired)
            {
                Invoke(new Action(() => OnQRCodeReceived(sender, e)));
                return;
            }

            try
            {
                Console.WriteLine($"[frmJobTH] QR Code received: {e.Code} for line: {e.LineName}");
                
                // Update the LineName textbox if received
                if (!string.IsNullOrEmpty(e.LineName))
                {
                    LineName.Text = e.LineName;
                }
                
                // Add code to DataGridView (only if not already exists)
                if (dgvReceivedData != null && !string.IsNullOrEmpty(e.Code))
                {
                    // Check if code already exists in the list
                    bool codeExists = false;
                    foreach (DataGridViewRow existingRow in dgvReceivedData.Rows)
                    {
                        if (existingRow.Cells["colCode"].Value?.ToString() == e.Code)
                        {
                            codeExists = true;
                            break;
                        }
                    ...
                    if (!codeExists)
                    {
                        int rowIndex = dgvReceivedData.Rows.Add();
                        DataGridViewRow row = dgvReceivedData.Rows[rowIndex];
                        row.Cells["colCode"].Value = e.Code;
                        
                        // Keep only last 100 entries to prevent memory issues
                        if (dgvReceivedData.Rows.Count > 100)
                        {
                            dgvReceivedData.Rows.RemoveAt(dgvReceivedData.Rows.Count - 1);
                        }
                        
                        // Auto-scroll to show latest entry
                        if (dgvReceivedData.Rows.Count > 0)
                        {
                            dgvReceivedData.FirstDisplayedScrollingRowIndex = 0;
                        }
                        
                        Console.WriteLine($"[frmJobTH] Added new code to list: {e.Code}");
                    }
                    else
                    {
                        Console.WriteLine($"[frmJobTH] Code already exists in list: {e.Code}");
                    }
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[frmJobTH] Error processing QR Code data: {ex.Message}");
            }
        }

        #endregion QR Code Data Display

        private void InitColumnNameControl()
        {
            if (InvokeRequired) { Invoke(new Action(() => InitColumnNameControl())); return; }

            // === Tính vị trí tham chiếu từ LineName / label1 đã có ===
            int labelLeft = label1?.Left ?? 10;
            int inputLeft = LineName?.Left ?? 120;
            int inputWidth = (cmbTableName != null && cmbTableName.Width > 50)
                             ? cmbTableName.Width
                             : (LineName?.Width ?? 250);
            int labelWidth = Math.Max(80, inputLeft - labelLeft - 5);
            int baseY = (LineName != null) ? LineName.Bottom + 10 : 110;

            // === 1. Di chuyển cmbTableName + lblTableName lên vùng hiển thị ===
            if (lblTableName != null)
            {
                lblTableName.AutoSize = false;
                lblTableName.Size = new System.Drawing.Size(labelWidth, 23);
                lblTableName.Location = new System.Drawing.Point(labelLeft, baseY + 2);
                lblTableName.BringToFront();
            }
            if (cmbTableName != null)
            {
                cmbTableName.Location = new System.Drawing.Point(inputLeft, baseY);
                cmbTableName.Size = new System.Drawing.Size(inputWidth,
                                            cmbTableName.Height > 0 ? cmbTableName.Height : 23);
                cmbTableName.BringToFront();
            }

            // === 2. Tạo lblColumnName + cmbColumnName ngay bên dưới ===
            int cmbTableHeight = cmbTableName?.Height ?? 23;
            int colY = baseY + cmbTableHeight + 8;

            lblColumnName = new System.Windows.Forms.Label();
            lblColumnName.Text = "Trường barcode:";
            lblColumnName.ForeColor = System.Drawing.SystemColors.WindowFrame;
            lblColumnName.AutoSize = false;
            lblColumnName.Size = new System.Drawing.Size(labelWidth, 23);
            lblColumnName.Location = new System.Drawing.Point(labelLeft, colY + 2);

            cmbColumnName = new System.Windows.Forms.ComboBox();
            cmbColumnName.DropDownStyle = System.Windows.Forms.ComboBoxStyle.DropDownList;
            cmbColumnName.FormattingEnabled = true;
            cmbColumnName.Name = "cmbColumnName";
            cmbColumnName.Location = new System.Drawing.Point(inputLeft, colY);
            cmbColumnName.Size = new System.Drawing.Size(inputWidth, 23);

            tabPage2.Controls.Add(lblColumnName);
            tabPage2.Controls.Add(cmbColumnName);
            lblColumnName.BringToFront();
            cmbColumnName.BringToFront();

            // === 3. Đẩy DataGridView xuống để không bị che ===
            if (dgvReceivedData != null)
            {
                int newTop = colY + 32;
                if (dgvReceivedData.Top < newTop)
                {
                    int diff = newTop - dgvReceivedData.Top;
                    dgvReceivedData.Location = new System.Drawing.Point(
                        dgvReceivedData.Left, newTop);
                    dgvReceivedData.Height = Math.Max(60, dgvReceivedData.Height - diff);
                }
            }

            // === 4. Wire event và load columns ngay nếu table đã được chọn ===
            cmbColumnName.SelectedIndexChanged += CmbColumnName_SelectedIndexChanged;
        ...
        {
            // Preview: có thể mở rộng sau
        }

        private void PopulateColumnNames(string tableName)
        {
            if (InvokeRequired) { Invoke(new Action(() => PopulateColumnNames(tableName))); return; }
            if (cmbColumnName == null || string.IsNullOrWhiteSpace(tableName)) return;
            ...
            try
            {
                ...
                foreach (string col in columns)
                    cmbColumnName.Items.Add(col);

                if (cmbColumnName.Items.Count > 0)
                    cmbColumnName.SelectedIndex = 0;

                Console.WriteLine($"[PopulateColumnNames] Table={tableName}, Columns={columns.Count}");
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[frmJobTH] Error loading columns: {ex.Message}");
            }
        }
