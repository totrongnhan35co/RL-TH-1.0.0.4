...
{
    public partial class FrmMainTH : Form
    {
        #region VARIABLES DEFINITION
        private readonly FrmJobTH _ParentForm = null;
        ...
        private FrmCheckedResultTH _FormCheckedResult;
        ...
        public FrmMainTH(FrmJobTH parentForm)
        {
            InitializeComponent();
        ...
        private void ActionChanged(object sender, EventArgs e)
        {
            ...
            else if (sender == pnlCheckFailed || sender == pnlCheckPassed || sender == pnlTotalChecked)
            {
                ...
                {
                    _FormCheckedResult = new FrmCheckedResultTH
                    {
            ...
            try
            {
                ...
                using (IDbConnection connection = GetDatabaseConnection(connectionString))
                {
                    ...
                    foreach (string codeWithDate in printedCodes)
                    {
                        ...
                        {
                            updateQuery = $"UPDATE [{tableName}] SET [qr_print] = 'Y', [qr_print_datetime] = @printedDate WHERE [{firstColumnName}] = @code";
                        }
                        else // MySQL
                        {
                            updateQuery = $"UPDATE `{tableName}` SET `qr_print` = 'Y', `qr_print_datetime` = @printedDate WHERE `{firstColumnName}` = @code";
