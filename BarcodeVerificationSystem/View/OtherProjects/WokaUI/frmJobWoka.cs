...
{
    public partial class FrmJobWoka : Form
    {
        ...
        private FrmViewLogFile _FormViewLogFile;
        ...
        private FrmMainWoka _FormMainPC = null;
            ...
            else if (sender == btnNext)
            {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        {
                            _FormMainPC = new FrmMainWoka(this);
                            _FormMainPC.Show();
                        }
                        else
                        {
        ...
        private void InitControls()
        {
            ...
            btnLog.Visible = currentUser == "Support";
            btnDelete.Visible = Shared.Settings.IsAllowJobDeletion;
            _LabelStatusCameraList.Add(lblStatusCamera01);
            UpdateStatusLabelCamera();
            _LabelStatusPrinterList.Add(lblStatusPrinter01);
        ...
        {
            _JobModel.NumberOfCodesInPallet = (int)codesInBoxNum.Value;
        }
        ...
        private void SaveJob()
        {
            try
            {
                ...
                if (_JobModel != null)   // Check current Job has null
                {
                    ...
                    if(_JobModel.NumberOfCodesInPallet <= 0)
                    {
                        CuzMessageBox.Show("Vui lòng nhập số lượng mã trong thùng hợp lệ!", Lang.Confirm, MessageBoxButtons.OK, MessageBoxIcon.Information);
                        return;
                    }
                            ...
                            if (cameraModel.IsEnable)
                            {
                                switch (cameraModel.CameraType)
                                {
                                    ...
                                    case CameraType.DM: // DM Series Camera
                                        ...
                                        break;
                                    case CameraType.HIKROBOT:
                                        // Nếu chưa có hoặc IP đã đổi thì disconnect và tạo lại
                                        ...
                                        if (Shared.hikrobotCamera == null)
                                        {
                                            ...
                                            if (opened)
                                            {
                                                ...
                                                Shared.hikrobotCamera.StartSdkGrab();
                                                // Gắn event nếu cần
                                            }
                                        }
                                        else
                                        {
                                            opened = Shared.hikrobotCamera.IsSdkOpen;
                                        }
        ...
        //// Add these helper methods inside the FrmJobWoka class (near other helpers)
        //private void AttachHikrobotHandlers(HikrobotCamera cam)
        //{
        //    if (cam == null) return;
        //    try
        //    {
        //        // Ensure no duplicate subscriptions
        //        cam.OnImageReceived -= Hikrobot_OnImageReceived;
        //        cam.OnQRCodeDecoded -= Hikrobot_OnQRCodeDecoded;

        //        cam.OnImageReceived += Hikrobot_OnImageReceived;
        //        cam.OnQRCodeDecoded += Hikrobot_OnQRCodeDecoded;
        //    }
        //    catch (Exception) { /* swallow to keep monitor loop alive */ }
        //}

        //private void DetachHikrobotHandlers(HikrobotCamera cam)
        //{
        //    if (cam == null) return;
        //    try
        //    {
        ...
        //private void Hikrobot_OnImageReceived(Bitmap bmp)
        //{
        //    try
        //    {
        //        if (bmp == null) return;
        //        // Clone the bitmap, Shared consumers will dispose if needed
        ...
        //        // Forward into existing pipeline
        //        Shared.RaiseOnCameraReadDataChangeEvent(detectModel);
        //    }
        //    catch (Exception) { /* non-fatal */ }
        //}

        //private void Hikrobot_OnQRCodeDecoded(string qr)
        //{
        //    try
        //    {
        //        var detectModel = new DetectModel
        //        {
        //            Text = qr ?? string.Empty,
        //            Image = null,
        //            Device = "Camera"
        //        };
        ...
        {
            if (_FormViewLogFile == null || _FormViewLogFile.IsDisposed)
            {
                _FormViewLogFile = new FrmViewLogFile();
                _FormViewLogFile.Show();
            }
            else
            {
                if (_FormViewLogFile.WindowState == FormWindowState.Minimized)
                    _FormViewLogFile.WindowState = FormWindowState.Normal;
                _FormViewLogFile.Focus();
                _FormViewLogFile.BringToFront();
            }
        }
    }
