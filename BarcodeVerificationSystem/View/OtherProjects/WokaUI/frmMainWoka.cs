...
{
    public partial class FrmMainWoka : Form
    {
        #region VARIABLES DEFINITION
        private readonly FrmJobWoka _ParentForm = null;
        ...
        private bool _isSendingDataToPrinter = false;
        ...
        private FrmCheckedResultWoka _FormCheckedResult;
        ...
        public FrmMainWoka(FrmJobWoka parentForm)
        {
            InitializeComponent();
        ...
        private void InitControls()
        {
            ...
            if (_cargoDataStorageProcess != null)
            {
                _cargoDataStorageProcess.Stop();
            }
            if (_cargoDataSenderProcess != null)
            {
                _cargoDataSenderProcess.Stop();
            }

            Shared.WokaAllValueProcess = null;
        ...
        private void InitEvents()
        {
            ...
            // Lắng nghe lỗi nghiêm trọng từ WokaCargoStorage (ví dụ: không thể map mã vào carton)
            WokaCargoStorage.OnFatalError += WokaCargoStorage_OnFatalError;
        ...
        private int GetSyncCodeInCarton()
        {
            try
            {
                int sentCartons = Shared.CurrentJob?.CartonList?.Count(x => x.IsCodeCartonSent) ?? 0;
                int palletSize = Shared.CurrentJob?.NumberOfCodesInPallet ?? 0;
                return sentCartons * palletSize;
            }
            catch (Exception) { }
            return 0;
        }
        ...
        private void SendVerifiedDataToPrinter(object sender, EventArgs e) // Verify and print 
        {
            ...
            if (Shared.Settings.VerifyAndPrintBasicSentMethod)
                command += WokaDataNormalizer.ToPrinterFormat(arr[1] == null ? Shared.Settings.FailedDataSentToPrinter : arr[1]);
            else
            {
                ...
                if (Shared.Settings.PrintFieldForVerifyAndPrint.Count() == 0)
                {
                    command += string.Join(Shared.Settings.SplitCharacter.ToString(), arr
                        .Select(x => WokaDataNormalizer.ToPrinterFormat(x == null ? Shared.Settings.FailedDataSentToPrinter : x)));
                }
                else
                {
                    command += string.Join(Shared.Settings.SplitCharacter.ToString(), Shared.Settings.PrintFieldForVerifyAndPrint
                        ...
                        .Select(x => WokaDataNormalizer.ToPrinterFormat(arr[x.Index - 1] == null ? Shared.Settings.FailedDataSentToPrinter : arr[x.Index - 1]))
                        );
                }
            }
        ...
        private void StartProcess(bool interactOnUI = true)
        {
            ...
            // ── [FIX] Khởi tạo WokaAllValueProcess một lần duy nhất trước khi các thread bắt đầu ──
            if (_SelectedJob.CompareType == CompareType.Database)
            {
                try
                {
                    if (Shared.WokaAllValueProcess == null)
                        Shared.WokaAllValueProcess = new WokaAllValueProcess(_SelectedJob);
                }
                catch (Exception ex)
                {
                    ProjectLogger.WriteError($"[WOKA INIT] Khởi tạo WokaAllValueProcess thất bại trong StartProcess | job={_SelectedJob?.FileName}", ex);
                }
            }
        ...
        private void UpdateUIPrintedResponse(CancellationToken token)
        {
            ...
            try
            {
                while (true)
                {
                    ...
                    if (podCommand != null)
                    {
                        // KHÔNG normalize printer response ở đây - sẽ normalize lúc lookup
        ...
        /// <summary>
        /// Ghi PrintedResponse raw — KHÔNG dùng Csv.Escape()
        /// vì InitPrintedStatusFromCsv chỉ cần cột Index (fields[0]) để cập nhật trạng thái
        /// Barcode GS1 chứa ký tự đặc biệt như " và ; sẽ được lưu nguyên bản
        /// </summary>
        private void SavePrintedResponseRaw(List<string[]> list, string path)
        {
            try
            {
        ...
        private void NewExportPrintedResponseToFile(CancellationToken token)
        {
            ...
            try
            {
                ...
                // Dùng Shared.WokaAllValueProcess đã được khởi tạo trong StartProcess
                if (Shared.WokaAllValueProcess == null)
                {
                    ProjectLogger.WriteError("[WOKA UPDATEPRINT] Shared.WokaAllValueProcess đang null, bỏ qua");
                }
                ...
                while (true)
                {
                    ...
                    if (valueArr.Count() > 0)
                    {
                        SavePrintedResponseRaw(valueArr, path);

                        // ── [FIX 3] Log dùng "qrcode" thay vì "field" ──────────────
                        try
                        {
                            string qrcode = clone[0][2];
                            string printedAt = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
                            ProjectLogger.WriteInfo($"[WOKA UPDATEPRINT] qrcode={qrcode} | printedAt={printedAt}");
                            Shared.WokaAllValueProcess.UpdatePrint(qrcode, printedAt);
                            ProjectLogger.WriteInfo($"[WOKA UPDATEPRINT OK] qrcode={qrcode}");
                        }
                        catch (Exception ex)
                        {
                            ProjectLogger.WriteError($"[WOKA UPDATEPRINT FAILED] qrcode={clone[0][2]}", ex);
                        }
                        // ─────────────────────────────────────────────────────────────
                    }
                    valueArr.Clear();
        ...
        //                try
        //                {
        //                    ProjectLogger.WriteInfo($"[SYNC UPDATEPRINT] field={clone[0][2]} | time={DateTime.Now:yyyy-MM-dd HH:mm:ss}");
        //                    Shared.WokaAllValueProcess.UpdatePrint(clone[0][2], DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"));
        //                    ProjectLogger.WriteInfo($"[SYNC UPDATEPRINT OK] field={clone[0][2]}");
        //                }
        //                catch (Exception ex)
        //                {
        //                    ProjectLogger.WriteError($"[SYNC UPDATEPRINT FAILED] field={clone[0][2]}", ex);
        //                }
        //            }
        //            valueArr.Clear();
        //            Thread.Sleep(5);
        ...
        private void NewExportCheckedResultToFile(CancellationToken token)
        {
            ...
            try
            {
                ...
                // ── [FIX] Khởi tạo WokaAllValueProcess TRƯỚC cargo processors để dùng chung 1 instance ──
                if (Shared.WokaAllValueProcess == null)
                {
                    ProjectLogger.WriteError("[WOKA INIT] Shared.WokaAllValueProcess đang null trong NewExportCheckedResultToFile");
                }

                // ── [FIX 1] Ghi log khi tạo processor, không nuốt exception ──────────
                try
                {
                    CreateWokaCargoStorageProcesssor(_SelectedJob);
                    CreateWokaCargoSenderProcesssor(_SelectedJob);
                }
                catch (Exception ex)
                {
                    ProjectLogger.WriteError($"[WOKA INIT] Khởi tạo CargoProcessor thất bại | job={_SelectedJob?.FileName}", ex);
                }
                // ─────────────────────────────────────────────────────────────────────
                ...
                while (true)
                {
                    ...
                    if (valueArr.Count() > 0)
                    {
                        // ── [FIX 2] Tách riêng từng try/catch để log đúng nguồn lỗi ─
                        if (clone[0][2] == "Valid")
                        {
                            // -- Enqueue vào cargo processors --
                            try
                            {
                                if (_cargoDataStorageProcess != null)
                                    _cargoDataStorageProcess.Enqueue(int.Parse(clone[0][0]), clone[0]);
                                else
                                    ProjectLogger.WriteWarning($"[WOKA STORAGE] _cargoDataStorageProcess is null, bỏ qua | index={clone[0][0]}");

                                if (_cargoDataSenderProcess != null)
                                    _cargoDataSenderProcess.Enqueue(int.Parse(clone[0][0]), clone[0]);
                                else
                                    ProjectLogger.WriteWarning($"[WOKA SENDER] _cargoDataSenderProcess is null, bỏ qua | index={clone[0][0]}");
                            }
                            catch (Exception ex)
                            {
                                ProjectLogger.WriteError($"[WOKA ENQUEUE FAILED] index={clone[0][0]} | qrcode={clone[0][1]}", ex);
                            }

                            // -- Cập nhật AllValues file --
                            try
                            {
                                if (Shared.WokaAllValueProcess != null)
                                {
                                    DateTime dt = DateTime.Now;
                                    Shared.WokaAllValueProcess.UpdateCheck(clone[0][1], dt.ToString("yyyy-MM-dd HH:mm:ss"));
                                    ProjectLogger.WriteInfo($"[WOKA UPDATECHECK OK] qrcode={clone[0][1]} | checkedAt={dt:yyyy-MM-dd HH:mm:ss}");
                                }
                                else
                                {
                                    ProjectLogger.WriteWarning($"[WOKA UPDATECHECK] WokaAllValueProcess is null, bỏ qua | qrcode={clone[0][1]}");
                                }
                            }
                            catch (Exception ex)
                            {
                                ProjectLogger.WriteError($"[WOKA UPDATECHECK FAILED] qrcode={clone[0][1]}", ex);
                            }
                        }
                        // ─────────────────────────────────────────────────────────────
        ...
        private void CreateWokaCargoStorageProcesssor(JobModel jobModel)
        {
            try
            {
                ...
                _cargoDataStorageProcess = ReliableProcessorFactory.CreateWokaCargoStorageProcessor(sentPalletDataPath, "", pathPallet);
                _cargoDataStorageProcess.Start();
                ProjectLogger.WriteInfo($"[STORAGE INIT] CargoStorageProcessor khởi động thành công | path={sentPalletDataPath}");
            }
            catch (Exception ex) // 
            {
                ProjectLogger.WriteError($"[STORAGE INIT FAILED] Không khởi động được CargoStorageProcessor | job={jobModel?.FileName}", ex);
            }
        }

        private VerificationQueueProcessor _cargoDataSenderProcess;

        private void CreateWokaCargoSenderProcesssor(JobModel jobModel)
        {
            try
            {
                ...
                _cargoDataSenderProcess = ReliableProcessorFactory.CreateWokaCargoSenderProcessor(sentPalletDataPath, "", pathPallet);
                _cargoDataSenderProcess.Start();

                ProjectLogger.WriteInfo($"[SENDER INIT] CargoSenderProcessor khởi động thành công | path={sentPalletDataPath}");
            }
            catch (Exception ex)
            {
                ProjectLogger.WriteError($"[SENDER INIT FAILED] Không khởi động được CargoSenderProcessor | job={jobModel?.FileName}", ex);
            }
        }
        ...
        private void StopProcess(bool interactOnUI = true, string messages = "", bool isClosed = false, bool isManualClose = false)
        {
            ...
            {
                ProjectLogger.WriteWarning($"[WOKA STOP] Dừng cargo processor thất bại: {ex.Message}");
            }
        }
        ...
        /// <summary>
        /// Xử lý lỗi nghiêm trọng từ WokaCargoStorage (ví dụ: không thể map mã vào carton sau nhiều lần retry).
        /// Giữ process in/check chạy, chỉ dừng cargo storage/sender để tránh gửi thiếu/thừa.
        /// </summary>
        private void WokaCargoStorage_OnFatalError(object sender, string message)
        {
            try
            {
                if (InvokeRequired)
                {
                    Invoke(new Action(() => WokaCargoStorage_OnFatalError(sender, message)));
                    return;
                }

                ProjectLogger.WriteError(message);

                // Dừng cargo storage/sender nhưng giữ process chính
                _cargoDataStorageProcess?.Stop();
                _cargoDataSenderProcess?.Stop();

                // Hiển thị cảnh báo cho operator
                CustomMessageBox.Show(
                    "Lỗi gom mã vào carton (Cargo Storage).\n" +
                    "Process in/kiểm tra vẫn chạy, nhưng không gom thùng cho đến khi được xử lý.\n\n" +
                    "Chi tiết: " + message,
                    Lang.Warning,
            ...
            {
                ProjectLogger.WriteError("[FRMMAIN] Lỗi trong WokaCargoStorage_OnFatalError: " + ex.Message);
            }
        }
        ...
        private void ActionChanged(object sender, EventArgs e)
        {
            ...
            else if (sender == pnlCheckFailed || sender == pnlCheckPassed || sender == pnlTotalChecked)
            {
                ...
                {
                    _FormCheckedResult = new FrmCheckedResultWoka
                    {
        ...
        private void Shared_OnCameraReadDataChange(object sender, EventArgs e)
        {
            ...
            try
            {
                if (sender is DetectModel)
                {
                    ...
                    // KHÔNG normalize camera data ở đây - sẽ normalize lúc so sánh trong DatabaseCompare
        ...
        {
            if (_isSendingDataToPrinter)
            {
                ProjectLogger.WriteWarning("[PRINTER] SendDataToPrinterAsync đang chạy, bỏ qua lần gọi thứ 2");
                return;
            }
        ...
        private void SendPODDataProductToPrinter(CancellationToken token)
        {
            ...
            try
            {
                ...
                for (int codeIndex = startIndex; codeIndex < codeList.Count(); codeIndex++)
                {
                    ...
                    {
                        // Pre-send guard: nếu mã đã được camera check thành công → đánh dấu Printed, không gửi lại
                        {
                            string[] rowCheck = codeModel.Where((item, idx) => idx != 1).ToArray();
                            string checkKey = GetCompareDataByPODFormat(rowCheck, _SelectedJob.PODFormat);
                            if (_CodeListPODFormat.TryGetValue(checkKey, out var cs) && cs.Status)
                            {
                        ...
                        //  string data = "";
                        token.ThrowIfCancellationRequested();
                        string data = string.Join(Shared.Settings.SplitCharacter.ToString(), codeModel.Skip(2).Select(x => WokaDataNormalizer.ToPrinterFormat(x ?? "")));// Init send data
                        //string command = string.Format("DATA;{0}", data); // Init send command
                        string command = $"DATA;{data}";

                       // ProjectLogger.WriteDebug($"[PRINTER SEND] codeIndex={codeIndex} | command={command}");
        ...
        private async void btnCompleteJob_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                var listCancelQr = Shared.WokaAllValueProcess.GetUnmappedQRCodes();
                ...
                LogToFile($"AccessToken present: {!string.IsNullOrEmpty(Shared.Settings.WokaToken)}");
