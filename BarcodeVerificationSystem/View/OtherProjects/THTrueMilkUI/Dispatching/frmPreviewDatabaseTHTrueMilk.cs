...
{
    public partial class FrmPreviewDatabaseTHTrueMilk : Form
    {
        ...
        public IList<string[]> _ObtainCodeList = new List<string[]>();
        public List<string> _DatabaseColunms = new List<string>();
        public int _Totals = 0;
        public int _NumberPrinted = 0;
        public bool _UseImageColumns = true; // false = hiển thị toàn text (dùng cho Manufacturing)
        ...
        public FrmPreviewDatabaseTHTrueMilk()
        {
            InitializeComponent();
        }
        ...
        private void UpdateGridRowCount()
        {
            ...
            dgvDatabase.RowCount = Math.Max(0, Math.Min(_PageRows, remaining));
        }
        ...
        private void RefreshPagination()
        {
           ...
           int pageStartIndex = 1;
            ...
            Invoke(new Action(() =>
            {
                // Populate GoToPage combobox — limit to avoid UI freeze with large datasets
                _isPopulatingCombo = true;
                comboBox1.Items.Clear();
                int maxComboItems = 10000;
                int totalItems = Math.Min(_PagesCount, maxComboItems);
                if (totalItems > 0)
                {
                    var pages = new object[totalItems];
                    for (int i = 0; i < totalItems; i++)
                        pages[i] = i + 1;
                    comboBox1.Items.AddRange(pages);
                }
        ...
        private void AssignColumnNameToTable(List<string> values, int imgIndex)
        {
            ...
            dgvDatabase.ScrollBars = ScrollBars.Both;
            ...
            dgvDatabase.CellValueNeeded += (obj, e) =>
            {
                try
                {
                    ...
                    if (_UseImageColumns && (e.ColumnIndex == 1 || e.ColumnIndex == 2 || e.ColumnIndex == 3 || e.ColumnIndex == 4))
                    {
                        if (e.ColumnIndex == 4)
                        {
            ...
            try
            {
                ...
                {
                    if (_UseImageColumns && (index == 1 || index == 2 || index == 3 || index == 4) && imgIndex != -1)
                    {
        ...
        private void cuzControlBox1_Click(object sender, EventArgs e)
        {
