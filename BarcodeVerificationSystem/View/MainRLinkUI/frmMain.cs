    ...
    public partial class FrmMain : Form
    {
        ...
        private void FrmMain_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (IsCloseButtonAction == true)
            {
                ...
                if (dialogResult == DialogResult.Yes)
                {
                    // FIX: Cleanup camera SDK và log history (bypass Exit() nên phải tự xử lý)
                    ...
                    try
                    {
                        LoggingController.SaveHistory(
                            Lang.Exit, Lang.LogOut, Lang.LogoutSuccessfully,
        ...
        private void FrmMain_FormClosed(object sender, FormClosedEventArgs e)
        {
            _ParentForm.isShowPopupDisConOneTime = false;
            ReleaseResource();
            // FIX: Chỉ show frmJob nếu nó còn sống (không phải đang exit app)
            // Tránh ObjectDisposedException khi frmJob đã bị Close() từ X button
        ...
        static long GetCurrentTimeInMilliseconds()
        {
            ...
            return now.ToUnixTimeMilliseconds();
                ...
                /// <summary>
        /// Tạo compare key từ ExtraFields của DetectModel theo thứ tự PODFormat (dùng cho OCROnly mode).
        /// </summary>
        private string BuildOcrCompareKey(DetectModel detectModel,
            List<OcrToolMapping> mappings, List<PODModel> podFormat)
        {
            // Xây mapping: PODName → giá trị OCR đọc được
            var ocrByPodName = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase);
            foreach (var m in mappings)
            {
                if (detectModel.ExtraFields != null &&
                    detectModel.ExtraFields.TryGetValue(m.ToolKey, out string v))
                {
                    ocrByPodName[m.PODName] = (v ?? "").Trim();
                }
            }

            // Ghép theo thứ tự PODFormat giống GetCompareDataByPODFormat
            var sb = new StringBuilder();
            foreach (var pod in podFormat)
            {
                if (pod.Type == PODModel.TypePOD.FIELD)
                {
                    if (ocrByPodName.TryGetValue(pod.PODName, out string val))
                        sb.Append(val);
                }
                else if (pod.Type == PODModel.TypePOD.TEXT)
                {
                    sb.Append(pod.Value);
                }
            }
        ...
        private void Compare(CancellationToken token) // Hàm so sánh dữ liệu
        {
            ...
            try
            {
                while (true)
                {
                    ...
                    if (Shared.Settings.IsItemsPerHour)
                    {
                        ...
                        if (itemsPerHour.InvokeRequired)
                        {
                            itemsPerHour.Invoke(new Action(() =>
                            {
                                itemsPerHour.Text = text;
                            }));
                        }
                        else
                        {
                    ...
                    if (detectModel != null)
                    {
                        ...
                        else if (_SelectedJob.CompareType == CompareType.Database)
                        {
                            ...
                            else
                            {
                                ...
                                // ── Hikrobot 3-mode compare ──────────────────────────────────────
                                var hikCamModel = Shared.Settings.CameraList
                                    .FirstOrDefault(c => c.CameraType == CameraType.HIKROBOT);
                                bool isHikOCR = hikCamModel?.OcrToolMappings?.Count > 0;

                                string compareKey = detectModel.Text;
                                if (isHikOCR && hikCamModel.HikrobotCompareMode == HikrobotCompareMode.OCROnly)
                                {
                                    compareKey = BuildOcrCompareKey(detectModel,
                                        hikCamModel.OcrToolMappings, _SelectedJob.PODFormat);
                                }

                                detectModel.CompareResult = isPosition && !isPositionCorrect
                                    ? ComparisonResult.Invalided
                                    : DatabaseCompare(compareKey, ref currentCheckedIndex);

                                // BarcodeAndOCR: sau khi barcode match, kiểm tra thêm từng cột OCR
                                if (detectModel.CompareResult == ComparisonResult.Valid
                                    && isHikOCR
                                    && hikCamModel.HikrobotCompareMode == HikrobotCompareMode.BarcodeAndOCR
                                    && detectModel.ExtraFields?.Count > 0
                                    && currentCheckedIndex >= 0)
                                {
                                    lock (_SyncObjCodeList)
                                    {
                                        string[] dbRow = _PrintedCodeObtainFromFile[currentCheckedIndex];
                                        foreach (var mapping in hikCamModel.OcrToolMappings)
                                        {
                                            if (string.IsNullOrEmpty(mapping.ToolKey) ||
                                                string.IsNullOrEmpty(mapping.PODName)) continue;
                                            if (!detectModel.ExtraFields.TryGetValue(
                                                mapping.ToolKey, out string ocrValue)) continue;
                                            var podEntry = _SelectedJob.PODFormat.FirstOrDefault(p =>
                                                p.Type == PODModel.TypePOD.FIELD &&
                                                string.Equals(p.PODName, mapping.PODName,
                                                    StringComparison.OrdinalIgnoreCase));
                                            if (podEntry == null) continue;
                                            // [0]=RowIdx [1]=Status [2..]=Field1... → colIdx = Index+1
                                            int colIdx = podEntry.Index + 1;
                                            if (colIdx >= dbRow.Length) continue;
                                            string dbVal = (dbRow[colIdx] ?? "").Trim();
                                            string camVal = (ocrValue ?? "").Trim();
                                            if (!string.Equals(dbVal, camVal, StringComparison.OrdinalIgnoreCase))
                                            {
                                                detectModel.CompareResult = ComparisonResult.Invalided;
                                                break;
                                            }
            ...
            {
                try { _UICheckedResultCancelTokenSource?.Cancel(); }
        ...
        private void UpdateUICheckedResult(CancellationToken token)
        {
            ...
            try
            {
                while (true)
                {
                    ...
                    strResult[Index_ResultData] = BuildDisplayText(detectModel);
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    if (valueArr.Count() > 0)
                    {
                        ...
                        try
                        {
                            if (clone[0][Index_Result] == "Valid" && Shared.Settings.MarkCheckedAsPrintedEnable)
                            {
        ...
        private List<string[]> InitDatabase(string path, bool isFirstRowHeader) // Hàm đọc file
        {
            Stopwatch stw = Stopwatch.StartNew();
